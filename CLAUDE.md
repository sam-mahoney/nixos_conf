...
- **`modules/nixos/`** — NixOS system modules (boot, networking, desktop, hardware, services, etc.)
- **`modules/nixos/hosts/`** — Host-specific hardware quirks (GPU drivers, kernel flags)
- **`modules/darwin/`** — nix-darwin system module and macOS defaults
- **`modules/home-manager/`** — User-level modules shared via `common.nix` baseline; Linux adds sway/noctalia/swaylock, Darwin adds aerospace
...
Tests live in `tests/` as Nix derivations that assert configuration properties. They are evaluated as part of `nix flake check`. Examples: kernel param validation, package presence checks, config state assertions.
...
- Monochrome theme (black/grey/white) is applied consistently across all UI components
